...
const FemaleTrainings = () => {
  ...
  const femaleTrainings = useMemo(() => {
    return trainings.filter((training) => training.gender === "female");
  }, [trainings]);

  return (
    <div className="w-full h-full max-w-screen-xl overflow-y-auto p-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {femaleTrainings.map((training, index) => (
          <div
            key={index}
        ...
        <CreateTrainingModal gender={"female"} />
      </div>
    </div>
  );
};

export default FemaleTrainings;
